...
| task_status__c | 작업 상태 | 선택 목록 | 보류 작업 상태: <br><br> 완료됨 (task_completed__c) <br><br> 완료되었으나 오류(task_completed_with_errors__c) <br><br> 실패(task_failed__c) |
| task_type__c | 작업 유형 | 선택 목록 | 보류 작업 유형: <br><br> 계약 이벤트 동기화(agreements_events_synchronization__c) <br><br> 계약 이벤트 처리(agreements_events_processing__c) |
