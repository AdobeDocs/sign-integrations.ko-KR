...
### 서명자 개체 만들기 {#create-signatory-object}
...
자격 증명 모음을 성공적으로 통합하기 위해 *Adobe Sign 통합 프로필* 가 만들어지고 사용 권한이 *Adobe Sign 관리 작업*. Adobe Sign 통합 프로파일은 시스템 계정에 할당되며 Vault API를 호출할 때 통합에서 사용됩니다. 이 프로필은 다음에 대한 권한을 허용합니다.
...
시스템 계정 사용자가 특정 문서 생명주기에 대한 Adobe Sign 관리 그룹에 속하도록 하려면 사용자 역할 설정 레코드를 만들어야 합니다.

## 응용 프로그램 역할 생성 {#create-application-roles}
...
Adobe Sign과의 통합을 구축하려면 관리자는 다음 두 개의 새 공유 문서 필드를 생성해야 합니다.
...
이러한 공유 필드는 Adobe 서명에 적합한 모든 문서 유형에 추가해야 합니다. 두 필드 모두 Adobe Sign 관리 그룹의 구성원만 값을 업데이트할 수 있는 특정 보안이 있어야 합니다.

![서명 필드 세부 정보 이미지](images/signature-field-details.png)

관리자는 기존 공유 필드를 추가해야 합니다. *저장소 오버레이 사용 안 함(disable_vault_overlays__v)* Adobe 서명에 적합한 모든 문서 유형에 대해 활성으로 설정합니다. 선택적으로, 필드에는 Adobe Sign 관리 그룹의 구성원만 해당 값을 업데이트할 수 있는 특정 보안이 있을 수 있습니다.
...
다음에 대한 설정을 완료한 후 [!DNL Veeva Vault] 및 Adobe Sign 관리 계정은 관리자가 미들웨어를 사용하여 두 계정 간에 연결을 만들어야 합니다. 추가 [!DNL Veeva Vault] Adobe Sign 계정 연결은 Adobe Sign Identity에 의해 시작된 다음 Veeva Vault ID를 저장하는 데 사용됩니다.
...
**1단계.** &#39;Adobe Sign 관리자 역할&#39;이라는 새 응용 프로그램 역할을 만듭니다.
...
**10단계** Adobe Sign 프로세스에 적합한 모든 문서 분류에 대해 &#39;Adobe Sign 문서&#39;라는 문서 유형 그룹을 추가합니다.

**11단계** 모든 구성이 완료되면 시스템은 Adobe Sign 프로세스에 적합한 모든 문서에 대해 DAC에 Adobe Sign 관리 사용자가 추가되었는지 확인해야 합니다. 이는 문서 유형 그룹을 &#39;Adobe Sign 문서&#39;로, 응용 프로그램 역할을 &#39;Adobe Sign 관리자 역할&#39;로 및 통합 사용자를 지정하는 적절한 사용자 역할 설정 레코드를 생성하여 수행됩니다.
...
**5단계** &#39;Adobe Sign 통합 프로필&#39;이라는 보안 프로필을 사용하여 통합 사용자 프로필을 하나 만들고 Adobe Sign 관리 그룹에 할당합니다.
